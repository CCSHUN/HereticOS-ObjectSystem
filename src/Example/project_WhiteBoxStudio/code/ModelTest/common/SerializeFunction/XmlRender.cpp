...
CXmlNode::CXmlNode(CXmlRender* pOwner, int iPos) : m_pOwner(pOwner), m_iPos(iPos), m_nAttributes(0)
{
}
...
CXmlRender::CXmlRender(LPCTSTR pstrXML)
{
    m_pstrXML = NULL;
    m_pElements = NULL;
    m_nElements = 0;
...
CXmlRender::~CXmlRender()
{
    Release();
}

bool CXmlRender::IsValid() const
{
    return m_pElements != NULL;
}

void CXmlRender::SetPreserveWhitespace(bool bPreserve)
{
    m_bPreserveWhitespace = bPreserve;
}

bool CXmlRender::Load(LPCTSTR pstrXML)
{
    Release();
    ...
    bool bRes = _Parse();
    if( !bRes ) Release();
	_tcscpy(m_szXmlPath, pstrXML);
	return bRes;
}

bool CXmlRender::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
#ifdef _UNICODE
    ...
    if (encoding == XMLFILE_ENCODING_UTF8)
    {
        ...
        ::WideCharToMultiByte( CP_ACP, 0, (LPCWSTR)w_str, nWide,(LPSTR)pByte, wide, NULL, NULL);
        m_pstrXML[wide] = _T('\0');
...
bool CXmlRender::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	if(_tcscmp(pstrFilename, m_szXmlPath) == 0 ) return TRUE;

	Release();
	_tcscpy(m_szXmlPath, pstrFilename);
//     CStdString sFile = CPaintManagerUI::GetResourcePath();
//     if( CPaintManagerUI::GetResourceZip().IsEmpty() ) 
	{
        //sFile += pstrFilename;
        HANDLE hFile = ::CreateFile(pstrFilename, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
void CXmlRender::Release()
{
    if( m_pstrXML != NULL ) free(m_pstrXML);
    if( m_pElements != NULL ) free(m_pElements);
    m_pstrXML = NULL;
    m_pElements = NULL;
	memset(m_szXmlPath, 0, sizeof(m_szXmlPath));
	memset(m_szErrorMsg, 0, sizeof(m_szErrorMsg));
	memset(m_szErrorXML, 0, sizeof(m_szErrorXML));

	m_nElements = 0;
	m_nReservedElements = 0;
}

void CXmlRender::GetLastErrorMessage(LPTSTR pstrMessage, SIZE_T cchMax) const
{
    _tcsncpy(pstrMessage, m_szErrorMsg, cchMax);
}

void CXmlRender::GetLastErrorLocation(LPTSTR pstrSource, SIZE_T cchMax) const
{
    _tcsncpy(pstrSource, m_szErrorXML, cchMax);
}

CXmlNode CXmlRender::GetRoot()
{
    if( m_nElements == 0 ) return CXmlNode();
    return CXmlNode(this, 1);
}

bool CXmlRender::_Parse()
{
...
bool CXmlRender::_Parse(LPTSTR& pstrText, ULONG iParent)
{
...
CXmlRender::XMLELEMENT* CXmlRender::_ReserveElement()
{
...
void CXmlRender::_SkipWhitespace(LPCTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') ) pstr = ::CharNext(pstr);
}

void CXmlRender::_SkipWhitespace(LPTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') ) pstr = ::CharNext(pstr);
}

void CXmlRender::_SkipIdentifier(LPCTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}

void CXmlRender::_SkipIdentifier(LPTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr = ::CharNext(pstr);
}

bool CXmlRender::_ParseAttributes(LPTSTR& pstrText)
{   
...
bool CXmlRender::_ParseData(LPTSTR& pstrText, LPTSTR& pstrDest, char cEnd)
{
...
void CXmlRender::_ParseMetaChar(LPTSTR& pstrText, LPTSTR& pstrDest)
{
...
bool CXmlRender::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
    // Register last error

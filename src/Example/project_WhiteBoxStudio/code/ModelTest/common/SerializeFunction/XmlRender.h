#ifndef __XMLRENDER_H__
#define __XMLRENDER_H__
...
class CXmlRender
{
    friend CXmlNode;
public:
    CXmlRender(LPCTSTR pstrXML = NULL);
    ~CXmlRender();

    bool Load(LPCTSTR pstrXML);
    bool LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding = XMLFILE_ENCODING_ASNI);
    bool LoadFromFile(LPCTSTR pstrFilename, int encoding = XMLFILE_ENCODING_ASNI);
    void Release();
    bool IsValid() const;
...
private:
    CXmlNode();
    CXmlNode(CXmlRender* pOwner, int iPos);

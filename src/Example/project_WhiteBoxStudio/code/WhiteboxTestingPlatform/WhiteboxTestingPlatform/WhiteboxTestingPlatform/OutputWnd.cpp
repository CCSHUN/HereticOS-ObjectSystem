...
void COutputWnd::WriteString(const CString &sInfo, int nSB_POSITION, BOOL bResetContent)
{
	//need to deal with "\r\n"
	int nFirstPos = 0, nLastPos = 0;
	int nCount = sInfo.GetLength();
	static int nRowCount = 0;
	if(m_wndOutputBuild.GetSafeHwnd()==NULL) return;
	...
	for (int i=1; i<nCount; i++){
		if (sInfo[i-1] == 0x0D &&  sInfo[i] == 0x0A){
			if (i != nFirstPos+1){
				//clear separator
				CString s2 = sInfo.Mid(nFirstPos, i-nFirstPos-1);
				if(s2.Find(PCLINT_REPORT_SEPARATORS) != -1){
					s2.Replace(PCLINT_REPORT_SEPARATORS, _T("   "));
					s2.Trim();
					s2 = CString(_T("--->")) + s2;
				}

				m_wndOutputBuild.AddString(s2);			
				nRowCount++;
			}
	...
	if (nSB_POSITION == SB_BOTTOM || (nRowCount%10) == 0){

...
IMPLEMENT_DYNAMIC(CAdminTreeView, CDockablePane)
...
BEGIN_MESSAGE_MAP(CAdminTreeView, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_CONTEXTMENU()
	ON_WM_PAINT()
...
void CAdminTreeView::FillFileView()
{
	theApp.m_WorkSpaceWndManger.m_UserDataModel.GetAdminUiTree(m_wndFileView);
}

void CAdminTreeView::OnContextMenu(CWnd* pWnd, CPoint point)
{
...
void CAdminTreeView::AdjustLayout()
{
	if (GetSafeHwnd() == NULL)
	{
		return;
...
void CAdminTreeView::OnSetFocus(CWnd* pOldWnd)
{

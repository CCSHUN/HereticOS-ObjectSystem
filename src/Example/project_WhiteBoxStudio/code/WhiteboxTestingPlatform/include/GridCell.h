...
class IFL_EXPORT_CLASS CGridCell : public CGridCellBase
{
...
public:
    ...
    virtual void  SetPrintLines(int nPrintLines)  { m_nPrintLines = nPrintLines; } 
    virtual void  SetMemo(LPCTSTR sMemo)         { m_sMemo = sMemo;  } 
    virtual void  SetName(LPCTSTR sName)         { m_sName = sName;  } 
	...
	virtual int			GetPrintLines()				{ return m_nPrintLines;  } 
    virtual LPCTSTR		GetMemo() const				{ return (m_sMemo.IsEmpty())? _T("") : LPCTSTR(m_sMemo); } 
    virtual LPCTSTR		GetName() const				{ return (m_sName.IsEmpty())? _T("") : LPCTSTR(m_sName); } 
	...
	//Button等风格的按钮使用
	virtual	BOOL		ExpandRect(int nRow, int nCol, CRect &rect);

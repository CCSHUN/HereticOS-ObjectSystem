...
#define GVN_BASE					1001
#define GVN_ROWCOLSIZE_CHANGE		GVN_BASE				// 行高列宽改变时
#define GVN_ROWCOLSIZE_CHANGING		GVN_BASE + 1			// 行高列宽正在改变时
#define GVN_GRIDPOS_CHANGE			GVN_BASE + 2			// 在逻辑显示方式下，移动表格边框时则改变整个表格的位置
...
class IFL_EXPORT_CLASS CGridCtrl : public CWnd
{
...
public:
	...
	void 		SetCellName(int nRow, int nCol, CString sName);

	CString		GetCellMemo(int nRow, int nCol);
	void 		SetCellMemo(int nRow, int nCol, CString sName);
	//end

	CGridCellBase*	GetCell(const CString &sName);
    ...
    BOOL		SetItemText(const CString &sName, LPCTSTR str);
	...
	CString		GetItemText(CString sName);
	...
	BOOL		SetItemSectionState(int nRow, int nCol, UINT state);
	void		ClearItemSectionState(int nRow, int nCol, int nState);
	//end add
	...
	BOOL		SetCellType(CString sName, CRuntimeClass* pRuntimeClass);
    BOOL 		SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass);
    BOOL 		SetDefaultCellType( CRuntimeClass* pRuntimeClass);
	CString		GetCellType(const CString &sName);
	CString		GetCellType(int nRow, int nCol);
...
public:
	...
	BOOL 		IsCellVisibleIncHide(int nRow, int nCol);
    BOOL 		IsCellEditable(int nRow, int nCol);
    BOOL 		IsCellEditable(CCellID &cell);
    BOOL 		IsCellEditable2(int nRow, int nCol);
    BOOL 		IsCellEditable2(CCellID &cell);
...
public:
	...
	// qy 2006-8-16
	// 计算被隐藏的单元格所属的单元格大小
	CCellRange	GetCellRange(int row,int col);
	// 计算能够显示的区域
	CCellRange	GetCellVisibleRange(int row,int col);
	//end

	//设置单元格的格线显示状态
	void		SetCellLines(int row, int col, UINT state);
	UINT		GetCellLines(int row, int col);
...
public:
	void		Clear();
	CCellID		SetFocusCell(int nRow, int nCol,BOOL bSingleLine);
    virtual		~CGridCtrl();
	void		SetCellCombine(int nRow, int nCol, int xNum, int yNum);
...
public:
	...
	void		SetAllowDraw(BOOL bAllowDraw) { m_bAllowDraw = bAllowDraw; }
	BOOL		GetAllowDraw() { return m_bAllowDraw; }
...
protected:
	...
	void		ValidateAndModifyCellContents(int nRow, int nCol, CString strText);
...
protected:
	...
	virtual void PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                    CDC *pDC, BOOL& bFirst, struct PRINT_POS_INFO_ *pPrintInfo);
    virtual void PrintColumnHeadings(CDC *pDC, CPrintInfo *pInfo, int nStartCol, int nEndCol, struct PRINT_POS_INFO_ *pPrintInfo);
    virtual void PrintHeader(CDC *pDC, CPrintInfo *pInfo);
    virtual void PrintFooter(CDC *pDC, CPrintInfo *pInfo);
    virtual void PrintRowButtons(CDC *pDC, CPrintInfo* /*pInfo*/, int nStartRow, int nEndRow, struct PRINT_POS_INFO_ *pPrintInfo);
#endif
...
public:
	void		  PrintPreview();
	BOOL		  ModifyPrintRect(CDC *pDC, int nRow, int nCol, CRect &rect, 
								int &nRowHideNum, int &nColHideNum, 
								int nRectRight, int nRectBottom,
								int nStartRow, int nEndRow, int nStartCol, int nEndCol);

// Attributes
protected:
    // General attributes
	BYTE		m_nCursorType;										//光标类型
    ...
    CCellID     m_LeftClickDownCell, m_SelectionStartCell, m_SelectionEndCell;
...
private:
	...
	CGridMgr	*m_pGridMgr;
	BOOL		m_bChanged;
	UINT		m_nTotalRowHeight;			//所有行总的逻辑高度
	int			m_nMapMode;

public:

	//获得或设置打印预览状态
	void			SetPrintPreviewStatus(BOOL bPrintPreviewStatus) { m_bPrintPreview = bPrintPreviewStatus; } 
	BOOL			GetPrintPreviewStatus() { return m_bPrintPreview; }

	//设置表格是否已经改变
	void			SetGridChanged(BOOL bChanged) { m_bChanged = bChanged; }
	BOOL			GetGridChanged() { return m_bChanged; }

	void			SetGridMgr(CGridMgr *pGridMgr) { m_pGridMgr = pGridMgr; }
	CGridMgr *		GetGridMgr() { return m_pGridMgr; }

	//add by zhangxin : 2007-06-27
	//设置单元格字体

	void			SetCellFont(int nRowNo, int nColNo, int nFontHeight = 11, BOOL bBold = FALSE, 
							const char* pFontName = "黑体");	
	const LOGFONT	*GetCellFont(int nRowNo, int nColNo){ return (LOGFONT *)NULL; }

	//设置和获得单元格的公式
	void			SetFormula(int nRowNo, int nColNo, const CString &sFormula);
	CString			GetFormula(int nRowNo, int nColNo);
	void			SetFormula(CString sPos, const CString &sFormula);
	CString			GetFormula(CString sPos);

	//根据单元格名字找到位置
	BOOL			GetPosByName(CString sName, int *pRowNo, int *pColNo);

	//根据单元格名字得到或设置单元格文本 
	CString			GetCellTextByName(const CString &sCellName);
	void			SetCellTextByName(const CString &sCellName, const CString &sText);

	//根据单元格名字得到或设置单元格文本 
	CString			GetCellMemoByName(const CString &sCellName);
	void			SetCellMemoByName(const CString &sCellName, const CString &sMemo);

	//设置所有单元格的名字 
	void			SetAllCellName();

	//单元格编辑后处理公式
	void			EndEditDealMath(int nRow, int nCol, CString &sText);

	//单元格焦点离开后自动计算
	void			KillFocusAutoCal(int nRow, int nCol);

	//清空所有的选区
	void			ClearCellSelectedMap();

	//清空所有的选区的边线
	void			ClearCellSelectedMap_Section();

	static BOOL		StrIsNumber(CString &sData);

	//设置光标类型
	void			SetCursorType(BYTE nCursorType = CURSOR_TYPE_EXCEL) { m_nCursorType = nCursorType; }
	BYTE			GetCursorType() { return m_nCursorType; }
...
protected:
	...
	//add by zhangxin : date 2007-09-27
	void UpdateSelection(CCellID selectionStartCell, CCellID selectionEndCell, int nCurStartRow,
								int nCurStartCol, int nCurEndRow, int nCurEndCol,
								int *pNewStartRow, int *pNewStartCol, int *pNewEndRow, int *pNewEndCol);
	//end add
